...
const fetchBooks = async () => {
  ...
  books.value = response.data.map(book => {
    ...
    return { ...book, authorName: `${author.name} ${author.surname}` };
  });
  isLoaded.value = true;
};
...
const goToAddBook = () => {
  router.push({ name: 'AdminNewBooks' });
};

const goToUpdate = (bookId) => {
  router.push({ name: 'AdminBooksUpdate', params: { id: bookId } });
};
</script>
    ...
    <table class="table" v-else>
      ...
      <tbody>
        <tr v-for="book in books" :key="book.id">
          ...
          <td>
            <button @click="goToDetails(book.id)" type="button" class="btn btn-primary btn-lg">📖 Voir</button>
            <button @click="goToAddBook()" type="button" class="btn btn-primary btn-lg">➕ Ajouter</button>
            <button @click="goToUpdate(book.id)" type="button" class="btn btn-primary btn-lg">♻️ MAJ</button>
          </td>
        </tr>
      </tbody>
    </table>
